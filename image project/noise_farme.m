...
% Last Modified by GUIDE v2.5 19-Dec-2017 00:03:37
...
% --- Executes on button press in s_and_P.
function s_and_P_Callback(hObject, eventdata, handles)
% hObject    handle to s_and_P (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
X = str2num(get(handles.pro,'String'));
Y = str2num(get(handles.num,'String'));
oldimage = imread(handles.ImagePath);
axes(handles.image_before);
imshow(oldimage);
newimage = salt_and_papper_noise(oldimage,X,Y);
...
% Hints: get(hObject,'String') returns contents of pro as text
...
axes(handles.image_before);
imshow(oldimage);
newimage = uniform(oldimage,X,Y);
axes(handles.image_after);
imshow(newimage);

% --- Executes on button press in gussain.
function gussain_Callback(hObject, eventdata, handles)
% hObject    handle to gussain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
X = str2num(get(handles.pro,'String'));
oldimage = imread(handles.ImagePath);
